...
const typeNum = (arr) => {
  let solution = arr.filter(type => typeof (type) === 'number');
  return solution;
};
...
const containsAnd = (arr) => {
  let solution = arr.filter(type => type.includes('and'));
  return solution;
};
...
const oddValues = (arr) => {
  let solution = arr.filter(num => num % 2);
  return solution;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let solution = arr.filter(value => !forbiddenValues.includes(value));
  return solution;
};
