...
const count = (target, input) => {
  return input.reduce((acc, curValue) => {
    const rowCount = curValue.reduce((innerAcc, innerCurVal) => {
...
const totalSum = (input) => {
  return input.reduce((acc, currVal) => acc + currVal.reduce((innerAcc, innerCurVal) => innerAcc + innerCurVal, 0), 0);
};
